...
def resolve_level_url(level_name):
    ...
    if re.match(r"^level-\d+$", normalized_name):
        return f"https://backrooms-wiki-cn.wikidot.com/{normalized_name}", "Heuristic (Regex)"
    ...
    return f"https://backrooms-wiki-cn.wikidot.com/{normalized_name}", "Fallback (Default)"

def check_level_urls(start, end):
    print(f"{'Level':<10} | {'Method':<20} | {'Status':<15} | {'Content Len':<12} | {'URL'}")
    print("-" * 100)
    ...
    for i in range(start, end + 1):
        ...
        print(f"{level_name:<10} | {method:<20} | {status_msg:<15} | {content_len:<12} | {url}")
        sys.stdout.flush()
