            ...
            for node, updates in chunk.items():
                ...
                if node == "event_node":
                    if updates.get("logic_event"):
                        ...
                        print(
                            f"✅ LOGIC EVENT TRIGGERED: {evt.name} (Die: {evt.die_type})"
                        )
                    else:
                        print(
                            "❌ No Logic Event generated by Event Node (might be narrative only)"
                        )

                    if updates.get("suggestions"):
                        # It is possible suggestions are generated here OR later after resolve loop
                        print(
                            f"Suggestions found in Event Node: {updates['suggestions']}"
                        )
