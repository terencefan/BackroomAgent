...
def generate_json_node(state: LevelAgentState):
    ...
    try:
        ...
        llm = get_llm()
        prompt_path = os.path.join(os.path.dirname(__file__), "prompts", "generate_json.prompt")
...
def extract_items_node(state: LevelAgentState):
    ...
    try:
        ...
        if isinstance(parsed_json, dict) and "findable_items" in parsed_json:
            ...
            items = []
            logs.append(f"Warning: Unexpected JSON format. Got keys: {parsed_json.keys() if isinstance(parsed_json, dict) else 'Not a dict'}")
