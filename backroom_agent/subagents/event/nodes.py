...
def generate_event_node(state: EventAgentState):
    ...
    # Load system prompt
    prompt_path = os.path.join(os.path.dirname(__file__), "prompts", "event_agent.prompt")
    system_prompt_template = load_prompt(prompt_path)
    if not system_prompt_template:
        system_prompt_template = "You are an event generator for the Backrooms. Create a spooky event."
