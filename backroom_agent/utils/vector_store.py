...
class SimpleVectorStore:
    def __init__(self, db_path: str = "./data/vector_store/item_vector_store.pkl", model_name: str = "all-MiniLM-L6-v2"):
    ...
    def update_index(self, file_paths: List[str]):
        ...
        print(f"Generating embeddings for {len(texts_to_embed)} items (Incremental update)...")
        ...
        if embedding_matrix is not None:
             ...
             if appends:
                 ...
                 embedding_matrix = np.vstack([embedding_matrix, np.array(new_vecs_list)])
...
def rebuild_vector_db(item_dir: str = "./data/item", db_path: str = "./data/vector_store/item_vector_store.pkl"):
    store = SimpleVectorStore(db_path=db_path)
    store.build_index(item_data_dir=item_dir)

def update_vector_db(file_paths: List[str], db_path: str = "./data/vector_store/item_vector_store.pkl"):
    """增量更新向量数据库"""
    store = SimpleVectorStore(db_path=db_path)
    store.update_index(file_paths)

def search_similar_items(query: str, k: int = 3, db_path: str = "./data/vector_store/item_vector_store.pkl") -> List[Dict]:
    store = SimpleVectorStore(db_path=db_path)
